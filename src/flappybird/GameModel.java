...
public class GameModel {
    ...
    private void generateGoldenBall() {
        ...
        do {
            ...
            int finalYPU = yPU;
            valid        = pipes.stream().noneMatch(
                pipe -> pipe.getBounds().intersects(new Rectangle(WIDTH, finalYPU, 40, 40)));
            attempts++;
    ...
    private void generateDefender() {
        ...
        if ( now - lastDefenderSpawnTime < 8000 )
            return;
        int     x0       = WIDTH + 100;
        boolean overlaps = pipes.stream().anyMatch(pipe -> Math.abs(pipe.getX() - x0) < 100);
        if ( !overlaps ) {
    ...
    private void generateMiniMessi() {
        ...
        if ( now - lastMiniSpawnTime < 10000 )
            return;
        powerUps.add(MiniMessi.getInstancia(WIDTH + 50, (int) (Math.random() * (HEIGHT - 50))));
